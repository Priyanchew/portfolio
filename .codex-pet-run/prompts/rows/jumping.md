Create a single horizontal sprite strip for the Codex app digital pet `sprocket` in the state `jumping`.
...
Output exactly 5 separate animation frames arranged left-to-right in one single row. Each frame must show the same pet: A tiny, playful digital fox companion.
