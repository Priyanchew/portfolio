...
export default async function Blog({
  ...
  const jsonLdContent = JSON.stringify({
    ...
    image: post.image ? `${DATA.url}${post.image}` : undefined,
    url: `${DATA.url}/blogs/${slug}`,
    author: {
      "@type": "Person",
      name: DATA.name,
  ...
  return (
    <section id="blog">
      ...
      <div className="flex justify-start gap-4 items-center">
        <Link href="/blogs" className="text-sm text-muted-foreground hover:text-foreground transition-colors border border-border rounded-lg px-2 py-1 inline-flex items-center gap-1 mb-6 group" aria-label="Back to Blogs">
      ...
      <nav className="mt-12 pt-8 max-w-2xl">
        <div className="flex flex-col sm:flex-row justify-between gap-4">
          {previousPost ? (
            <Link
              href={`/blogs/${getSlug(previousPost)}`}
