...
[Live site](https://www.priyanchew.dev/) · [Branch preview](https://www.priyanchew.dev/new/) · [Preview branch](https://github.com/Priyanchew/portfolio/tree/new)
...
```sh
git clone --branch master https://github.com/Priyanchew/portfolio.git
cd portfolio
corepack enable
...
| [`src/components/portfolio/local-time.tsx`](src/components/portfolio/local-time.tsx) | Live local time, formatted for `Asia/Kolkata` and shown as “it’s <time> for me”. |
...
## Main site and branch preview on GitHub Pages

The deployment combines two independent builds into one Pages artifact:

- `master` is the main portfolio at `https://www.priyanchew.dev/`.
- `new` remains available at `https://www.priyanchew.dev/new/` for previewing branch changes.
...
The redesign is now merged into `master`. The workflow file is kept in sync on both branches, and each branch is still built separately. Future changes on `new` appear in the preview; merge them into `master` to publish them on the main homepage.

## Icons and search metadata

The default favicon is the olive `p.` monogram. [Compare all three options](docs/favicon-options.png). `pnpm icons:generate` exports SVG, PNG sizes from 16 to 512 pixels, a multi-size ICO, and an Apple touch icon. Both builds use the same icons.

Canonical and social URLs use `https://www.priyanchew.dev`, matching the domain redirect. Each public page has its own social title, description, and URL. The card is served at `/social-card.png` (or `/new/social-card.png` for the preview) so GitHub Pages supplies `image/png`; the extensionless `opengraph-image` route remains available for existing links.

The manifest keeps icons and home-screen bookmarks under the deployment base path. The local clock renders a neutral placeholder in the static export and updates in the browser, so the build time never appears as the current time.

`src/app/robots.ts` and `src/app/sitemap.ts` generate the crawler files. The main sitemap lists the root and `/new/` portfolios and their writing pages; the preview exports its own sitemap. Do not add matching files under `public/`, because they would conflict with these generated routes. Private document paths are excluded from the sitemap and blocked by the root crawler rules; the private index retains `noindex, nofollow`. Robots rules are crawler guidance, not access control.
